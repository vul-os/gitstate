...
import { BarList, type BarListItem } from '../components/ui/BarList.tsx'
import { SegmentedControl } from '../components/ui/SegmentedControl.tsx'
import { PageHeader, Spinner, ErrorState, EmptyState } from '../components/common.tsx'
import { useAsync } from '../lib/hooks.ts'
import { listRepos, healthMetrics } from '../lib/api.ts'
import { RANGES, formatHours, compact } from '../lib/analyticsView.ts'
import type { BusFactor, Dora, EngHealth as EngHealthPayload, Quality, Repo, ReviewHealth } from '../lib/types.ts'
...
interface LoadResult {
  repos: Repo[]
  health: EngHealthPayload
}

async function load(days: number, repoId: string): Promise<LoadResult> {
...
function BusFactorPanel({ busFactor }: { busFactor: Partial<BusFactor> }) {
  ...
  const items: BarListItem[] = contributors.map((c) => ({
    key: c.email,
...
function ReviewPanel({ review }: { review: Partial<ReviewHealth> }) {
...
function QualityPanel({ quality }: { quality: Partial<Quality> }) {
...
export default function EngHealth() {
  ...
  const dora: Partial<Dora> = health?.dora ?? {}
  const busFactor: Partial<BusFactor> = health?.bus_factor ?? {}
  const review: Partial<ReviewHealth> = health?.review ?? {}
  const quality: Partial<Quality> = health?.quality ?? {}
