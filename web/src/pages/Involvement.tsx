...
import { listRepos, involvement } from '../lib/api.ts'
import { RANGES } from '../lib/analyticsView.ts'
import type { Involvement as InvolvementPayload, PersonInvolvement, Repo, RepoInvolvement } from '../lib/types.ts'

type ViewMode = 'repo' | 'person'
...
function RepoCard({ repo }: { repo: RepoInvolvement }) {
...
function PersonCard({ person }: { person: PersonInvolvement }) {
  const items: BarListItem[] = useMemo(
    () => (person.repos ?? []).map((r) => ({
      key: r.repo_id,
      label: r.slug || '—',
...
export default function Involvement() {
  ...
  const [view, setView] = useState<ViewMode>('repo')
