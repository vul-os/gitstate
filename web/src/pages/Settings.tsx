...
import { health, syncStatus } from '../lib/api.ts'
import { useTheme } from '../lib/theme.tsx'
import type { HealthResp, SyncStatus } from '../lib/types.ts'
...
async function loadStatus(): Promise<StatusPayload> {
  const [h, sync] = await Promise.all([
    health(),
    syncStatus().catch((): SyncStatus => ({ enabled: false, peers: 0, peer_id: '', last_op_hlc: null })),
  ])
  return { h, sync }
}

function Row({ icon: Icon, title, children }: { icon: LucideIcon; title: ReactNode; children?: ReactNode }) {
  return (
