...
import { listRepos, addRepo, scanRepo, deleteRepo } from '../lib/api.ts'
import type { Forge, Repo } from '../lib/types.ts'

const FORGE_META: Record<Forge, { icon: ReactNode; label: string }> = {
  github: { icon: <Cloud size={12} />, label: 'GitHub' },
  gitlab: { icon: <Cloud size={12} />, label: 'GitLab' },
  local: { icon: <HardDrive size={12} />, label: 'Local' },
}

function ForgeBadge({ forge }: { forge: Forge }) {
  const f = FORGE_META[forge] ?? FORGE_META.local
  return <Badge color="default">{f.icon}{f.label}</Badge>
}

function AddRepoForm({ onAdded }: { onAdded?: () => void }) {
  const [mode, setMode] = useState<'path' | 'remote'>('path')
