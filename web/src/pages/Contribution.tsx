 ...
 */
function recompute(dimensions: Dimensions | undefined, w: Weights | null): number {
  let num = 0
  let den = 0
 ...
 */
function RepoChips({ repos, max = 2 }: { repos?: string[]; max?: number }) {
...
interface RankedRow extends ContributionRollupRow {
  liveComposite: number
}

function ContributorTexture({ rows, localWeights }: { rows: ContributionRollupRow[]; localWeights: Weights | null }) {
...
export default function Contribution() {
  ...
  const [local, setLocal] = useState<Weights | null>(null)
  ...
  function setWeight(key: keyof Weights, value: number) {
    setLocal({ ...(effective as Weights), [key]: value })
  }
