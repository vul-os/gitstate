...
import { Heatmap } from '../components/ui/Heatmap.tsx'
import { TrendChart } from '../components/ui/TrendChart.tsx'
import { PageHeader, Spinner, ErrorState, EmptyState, MetricPill } from '../components/common.tsx'
...
import { commitSeries, cycleSeries, sparkOf, formatHours, compact } from '../lib/analyticsView.ts'
import type {
  Analytics, Contribution, Contributor, Dimensions, ProjectState, Repo, WorkItem, WorkState,
} from '../lib/types.ts'

const DIMS: [keyof Dimensions, string][] = [
  ['shipped', 'Shipped'],
  ['review', 'Review'],
  ['effort', 'Effort'],
  ['quality', 'Quality'],
...
async function loadRepo(id: string): Promise<RepoDetailData> {
...
function ActivityPanel({ stats }: { stats: Analytics | null }) {
...
function DimBar({ value }: { value: number | null | undefined }) {
...
function ProjectStatePanel({ state }: { state: ProjectState | null }) {
  if (!state) {
    return (
      <EmptyState
...
function ContributionsTable({ contribs, people }: { contribs: Contribution[]; people: Contributor[] }) {
...
const STATE_COLOR: Record<WorkState, BadgeColor> = {
  open: 'green', merged: 'indigo', closed: 'red', draft: 'default', in_progress: 'yellow', done: 'teal',
}
        ...
        <Card key={it.id} padding="sm" className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <Badge color={STATE_COLOR[it.state] || 'default'}>{it.kind}</Badge>
            </div>
            <p className="truncate text-sm text-[var(--text)]">{it.title}</p>
          </div>
          <Badge color={STATE_COLOR[it.state] || 'default'}>{it.state}</Badge>
        </Card>
      ))}
    </div>
  )
