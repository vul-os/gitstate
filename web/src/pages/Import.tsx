...
import { SegmentedControl, type SegmentedOption } from '../components/ui/SegmentedControl.tsx'
...
import type {
  ImportResp, ImportedItem, PreviewResp, Repo, TrackerKind, TrackerView,
} from '../lib/types.ts'

type ImportMode = 'connect' | 'file'

const MODES: SegmentedOption<ImportMode>[] = [
...
function PreviewTable({ items }: { items?: ImportedItem[] }) {
...
const EMPTY_FORM: TrackerForm = { base_url: '', email: '', token: '', project: '' }

function ConnectMode({ repos, configured, reload }: { repos: Repo[]; configured: TrackerView[]; reload: () => void }) {
  const [kind, setKind] = useState<TrackerKind>('jira')
  const meta = TRACKERS.find((t) => t.kind === kind)!
  const stored = configured.find((t) => t.kind === kind) ?? ({} as Partial<TrackerView>)
  ...
  function pick(next: TrackerKind) {
    setKind(next)
    setForm(EMPTY_FORM)
    setReplacing(false)
    setStatus(null)
  ...
  const set = (k: keyof TrackerForm) => (e: ChangeEvent<HTMLInputElement>) => setForm((f) => ({ ...f, [k]: e.target.value }))
...
function FileMode({ repos }: { repos: Repo[] }) {
