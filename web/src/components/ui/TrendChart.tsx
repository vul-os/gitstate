 ...
 */
import { useMemo, useState, useId, type MouseEvent, type ReactNode } from 'react'
import { useMeasure } from '../../lib/useMeasure.ts'
import type { Series } from '../../lib/analyticsView.ts'
...
export interface TrendChartProps {
  series?: Series[]
  height?: number
  /** Gradient under-fill (single series only). */
  area?: boolean
  yFormat?: (n: number) => string
  /** Axis tick text. */
  xFormat?: (x: string, index: number) => ReactNode
  /** Appended in the tooltip. */
  valueSuffix?: string
  emptyLabel?: string
}
...
export function TrendChart({
  ...
  const toX = (i: number) => PAD.left + (n === 1 ? plotW / 2 : (plotW / (n - 1)) * i)
  const toY = (v: number | null | undefined) => PAD.top + plotH - (max === 0 ? 0 : ((v ?? 0) / max) * plotH)
