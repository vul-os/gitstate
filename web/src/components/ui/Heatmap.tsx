...
import type { DayBucket } from '../../lib/types.ts'

export type HeatmapMetric = 'commits' | 'additions'

export interface HeatmapProps {
  days?: DayBucket[]
  metric?: HeatmapMetric
  onSelect?: (day: DayBucket) => void
}
...
export function Heatmap({ days = [], metric = 'commits', onSelect }: HeatmapProps) {
  const titleId = useId()
  const [hover, setHover] = useState<DayBucket | null>(null)
  const [ref, containerWidth] = useMeasure<HTMLDivElement>()

  const { columns, scale, monthTicks, total } = useMemo(() => {
    if (!days.length) {
      return { columns: [] as (DayBucket | null)[][], scale: () => 0, monthTicks: [] as MonthTick[], total: 0 }
    }
