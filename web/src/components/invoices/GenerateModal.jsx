/**
 * GenerateModal — the "Generate from git" flow.
 *
 * Pick a client (or project) + date range + rate → preview the line items that
...
export default function GenerateModal({ clients, projects, onClose, onCreated }) {
  ...
  // Rate in dollars for the input; converted to cents for the API.
  const selectedClient = useMemo(
    () => clients?.find((c) => c.id === clientId),
    [clients, clientId],
  )
  const defaultRate = selectedClient ? selectedClient.rateCents / 100 : 150
  const [rate, setRate] = useState(defaultRate)
  // Track if the user has manually edited the rate so switching client can refresh it.
  ...
  async function runPreview() {
    ...
    setError(null)
    try {
      const data = await generateInvoice(body(true))
      setPreview(data)
    } catch (e) {
  ...
  async function save() {
    ...
    setError(null)
    try {
      const inv = await generateInvoice(body(false))
      onCreated?.(inv)
    } catch (e) {
  ...
  const totalPoints = lines.reduce((s, l) => s + (l.effortPoints ?? 0), 0)
      ...
      >
          ...
          {preview && (
            <div className="rounded-[var(--radius-card)] overflow-hidden" style={{ border: '1px solid var(--border)' }}>
              <div className="px-4 py-2.5 flex items-center justify-between" style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
                <span className="text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-widest">Preview · {lines.length} line{lines.length !== 1 ? 's' : ''}</span>
                <span className="text-[11px] font-mono text-[var(--text-faint)]">{totalPoints.toFixed(1)} effort pts</span>
              </div>
              {lines.length === 0 ? (
                <div className="px-4 py-8 text-center text-xs text-[var(--text-faint)]">
                  <GitMerge size={20} className="mx-auto mb-2 opacity-50" />
                  No merged PRs found in this window. Try a wider date range or sync more activity.
                </div>
              ) : (
                    ...
                    return (
                      <div key={i}>
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-[var(--bg-surface-2)] transition-colors"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-[var(--text-faint)] mt-0.5">{(l.effortPoints ?? 0).toFixed(1)} pts × {format(Number(rate))}</p>
                          </div>
              ...
              <div className="px-4 py-3 flex items-center justify-between" style={{ background: 'var(--bg)', borderTop: '1px solid var(--border)' }}>
                <span className="text-xs font-semibold text-[var(--text-muted)]">Estimated total</span>
        ...
        <div className="px-6 py-4 flex items-center justify-between gap-3" style={{ borderTop: '1px solid var(--border)' }}>
          <p className="text-[10px] text-[var(--text-faint)] flex items-center gap-1.5">
            <FileText size={12} /> Saved as a draft — review before sending.
          </p>
