 ...
 * Insights screens.
 */
import type { Analytics, ThroughputPoint, WeekBucket } from './types.ts'

/** A single chart point — `x` is the pre-formatted axis label. */
export interface Point {
  x: string
  y: number
}

/** One named line in a multi-series `TrendChart`. */
...
type AnalyticsLike = Analytics | null | undefined

/** The `WeekBucket` fields that hold a plain numeric count. */
export type WeekNumericField = 'commits' | 'additions' | 'deletions'
 ...
 */
function wholeWeeks(a: AnalyticsLike): WeekBucket[] {
 ...
 */
export function cycleSeries(a: AnalyticsLike, limit = 120): Point[] {
...
export function throughputSeries(a: AnalyticsLike): Series[] {
  ...
  const all: ThroughputPoint[] = a?.throughput ?? []
 ...
 */
export function sparkOf(a: AnalyticsLike, field: WeekNumericField = 'commits', n = 14): number[] {
 ...
 */
export function trendDelta(a: AnalyticsLike, field: WeekNumericField = 'commits'): number | null {
  const weeks = wholeWeeks(a)
  if (weeks.length < 4) return null
  const mid = Math.floor(weeks.length / 2)
  const sum = (arr: WeekBucket[]) => arr.reduce((t, w) => t + (w[field] ?? 0), 0)
