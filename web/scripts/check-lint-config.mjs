 ...
 *
 * ---------------------------------------------------------------------------
 * PORTABILITY — this script is the canonical copy; it will be dropped into
 * 12 sibling repos. Everything that varies per repo lives in the CONFIG
 * block directly below. See the end-of-file comment for what each kind of
 * sibling repo is expected to change.
 * ---------------------------------------------------------------------------
 */
...
const CONFIG = {
  ...
  // resolves. For gitstate, tsconfig.json has `"include": ["src"]`. If a repo
  // has multiple tsconfigs/roots (e.g. pier's client/ and console/), point
  // this at one of them and duplicate CONFIG per root (see footer note).
  fixtureDir: 'src',
  fixtureName: '__lint_config_gate_probe.ts',

  // Repo root to walk for package.json files (assertion B). This is allowed
  // to reach outside webRoot — gitstate has package.json files at the repo
  // root, apps/desktop/, and scripts/ too, and a stray range on any of them
  // is worth catching even though only web/ has a typescript dependency.
  ...
  // (assertion C). See the reasoning in the header comment above main().
  coverageFloor: 40,
}
...
// ============================================================================
// Footer — what a sibling repo needs to change to adopt this script.
// ============================================================================
// - Frontend not under web/: change CONFIG.webRoot (and SCRIPT_DIR-relative
//   math if the script also moves). repoRoot should still point at the repo
//   root for assertion B's package.json sweep.
// - pier (two roots, client/ + console/, different package managers): run
//   two independent copies of this script (or wrap it in a loop) with
//   webRoot/fixtureDir/coverageFloor set per root; assertion B's repoRoot
//   sweep only needs to run once since it walks the whole tree regardless.
// - envoir (buildless JS, small TS slice): fixtureDir must point at whatever
//   directory tsconfig's "include" actually covers for the TS slice — if
//   there is no tsconfig/type-aware ESLint config at all yet, assertion A
//   does not apply; drop it and keep B (pin hygiene is still real) and a
//   coverage floor sized to the much smaller file count.
// - kotva / llmux (Node libraries, no framework): no react-hooks/jsx, but
//   assertions A/B/C are unchanged in kind. coverageFloor needs to be
//   re-measured from `eslint .`'s real current count, per repo — never
//   copy gitstate's 40.
// - Any repo without a tsconfig "include" scoping fixtureDir the same way:
//   confirm where projectService actually resolves files (Assertion A's
//   whole point) before trusting the fixture placement; a probe fixture
//   placed outside tsconfig's include silently fails to get type info and
//   the assertion will (correctly) fail — that's the "critical subtlety"
//   from the task, not a bug in the script.
